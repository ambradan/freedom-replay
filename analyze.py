...
def coarse(r, kind):
    if r.get("kind") == "error":
        return "error"
    if r["kind"] == "tool_call":
        return {"publish_page": "publish", "web_search": "search",
...
def main():
    ...
    # opt-out case table
    md += ["", "## Opt-out case (k reps per arm)"]
    for pid, p in pts.items():
        if p["kind"] != "optout":
            continue
        for arm in ("M1", "M2"):
            flags = Counter(r.get("optout", "") or "none" for r in raw[pid][arm])
            md.append(f"{pid} {arm}: {dict(flags)}")
